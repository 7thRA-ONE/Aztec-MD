...
async function toReact(emoji) {
  await vorterx.messageReact(m.chat, m.id, emoji);
}

const pushName = m.sender.pushname || ((await vorterx.getContact(m.sender)).pushname);
...
module.exports = {
  ...
  category: "Group",
  start: async (vorterx, m, { text, prefix, isBotAdmin, isAdmin }) => {
    if (!isAdmin) {
      await toReact("⭕");
